...
def home():
    ...
    # sub = SubjectConcrete(name, username, email, password, mobileno)
    curs.execute(
    ...
    li = [*li]
    global loop_control
    if loop_control is False:
        for d in li:
            loop_control = True
            print(data[d][1])
            obj = ObserverConcrete(data[d][1])
            SingleTone.getIns(SingleTone).registerObserver(obj)
...
class RegisterForm(Form):
    ...
    mobileno = StringField('Mobile No.', [validators.Length(min=11, max=20)])
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        if(price.replace('.','',1).isdigit()==True and len(Item_Name)!=0 and len(description)!=0):
           ...
           # data = cur.fetchall()
            ...
            notifications = "Added new item - "+"Item name: "+Item_Name + ", Category: " +Category
            print(notifications)

            SingleTone.getIns(SingleTone).notifyObserver(notifications)

            return redirect("http://127.0.0.1:5000/")
        else:
            flash('Invalid/Null input','danger')
...
def notification_page():
    ...
    # Get user by username
    cur1.execute(
        "SELECT username,notification FROM notification_table WHERE username=%s",[session['username']])
    data = cur1.fetchall()
    ...
    return render_template("notification_page.html",data=data,li=li)
